...
describe('Core V2 activity notification contracts', () => {
  ...
  it('preserves activity-event causation and the original correlation in the supplier event', () => {
    ...
    const event = ActivityNotificationRequestEmittedEventV2Schema.parse({
      actorPlayerId: request.activityItem.playerId,
      aggregateId: request.activityItem.activityItemId,
      aggregateType: 'activity_item',
      aggregateVersion: request.activityItem.version,
      causationId: request.sourceEventId,
      correlationId: request.correlationId,
      eventId: '49000000-0000-4000-8000-000000000099',
    ...
    expect(event.causationId).toBe(request.sourceEventId);
    expect(event.payload.request.causationId).toBe(request.causationId);
    expect(() =>
      ActivityNotificationRequestEmittedEventV2Schema.parse({
        ...event,
