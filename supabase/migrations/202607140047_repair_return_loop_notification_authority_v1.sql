-- Repair Return Loop notification delivery at the authority boundary.
-- The original enqueue block used PL/pgSQL variables named exactly like target
-- columns. PostgreSQL could reject the ambiguous expression; the defensive
-- delivery-error catch then preserved the notification while silently losing
-- its push job. Distinct variable names keep the durable notification-first
-- behavior without weakening failure isolation.
...
-- The notifications RLS policy invokes this SECURITY DEFINER identity helper
-- for authenticated reads, so callers need EXECUTE without table privileges.
revoke all on function private.current_return_loop_player_id_v1()
  from public, anon;
grant execute on function private.current_return_loop_player_id_v1()
  to authenticated;
