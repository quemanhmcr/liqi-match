-- Atomic identity-section profile read/write seam. Canonical profile version is
-- authoritative; legacy tables remain the projection consumed by existing UI.

create or replace function private.profile_identity_snapshot_v1(
  p_player_id uuid
)
returns jsonb
language plpgsql
...
declare
  ...
  gender_value text;
  status_value text;
  matches_value integer := 0;
  rating_value numeric := 0;
  reputation_value integer := 0;
  win_rate_value integer := 0;
begin
  ...
  where profile_id = canonical_profile_row.legacy_profile_id;
  ...
  gender_value := media_summary_value #>> '{profile_basics,gender}';
  if gender_value not in ('male', 'female', 'hidden') then
    gender_value := null;
  end if;

  status_value := media_summary_value->>'profile_status';
  if status_value not in ('ready', 'busy', 'offline', 'friends') then
    status_value := null;
  end if;

  if jsonb_typeof(media_summary_value #> '{profile_stats,matches}') = 'number' then
    matches_value := greatest(
      0,
      least(
        99999,
        round((media_summary_value #>> '{profile_stats,matches}')::numeric)::integer
      )
    );
  end if;

  if jsonb_typeof(media_summary_value #> '{profile_stats,rating}') = 'number' then
    rating_value := greatest(
      0,
      least(
        5,
        round((media_summary_value #>> '{profile_stats,rating}')::numeric, 1)
      )
    );
  end if;

  if jsonb_typeof(media_summary_value #> '{profile_stats,reputation}') = 'number' then
    reputation_value := greatest(
      0,
      least(
        100,
        round((media_summary_value #>> '{profile_stats,reputation}')::numeric)::integer
      )
    );
  end if;

  if jsonb_typeof(media_summary_value #> '{profile_stats,win_rate}') = 'number' then
    win_rate_value := greatest(
      0,
      least(
        100,
        round((media_summary_value #>> '{profile_stats,win_rate}')::numeric)::integer
      )
    );
  elsif jsonb_typeof(media_summary_value #> '{profile_stats,winRate}') = 'number' then
    win_rate_value := greatest(
      0,
      least(
        100,
        round((media_summary_value #>> '{profile_stats,winRate}')::numeric)::integer
      )
    );
  end if;
  ...
  return jsonb_build_object(
    'identity', jsonb_build_object(
      'bio', coalesce(legacy_profile_row.bio, ''),
      'displayName', legacy_profile_row.display_name,
      'genderId', gender_value,
...
create or replace function public.get_own_player_profile_identity_v1()
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  return private.profile_identity_snapshot_v1(player_id_value);
end;
$$;
...
begin
  ...
  media_summary_value := media_summary_value || jsonb_build_object(
    'profile_basics',
    ...
    'profile_stats', jsonb_build_object(
      'matches', matches_numeric::integer,
      'rating', round(rating_numeric, 1),
      'reputation', reputation_numeric::integer,
      'win_rate', win_rate_numeric::integer
    )
  );
...
revoke all on function private.profile_identity_snapshot_v1(uuid)
  from public, anon, authenticated;
revoke all on function public.get_own_player_profile_identity_v1()
  from public, anon, authenticated;
revoke all on function public.update_player_profile_identity_v1(jsonb)
  from public, anon, authenticated;
grant execute on function public.get_own_player_profile_identity_v1()
