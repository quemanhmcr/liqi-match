...
const accountDelete = fs.existsSync(accountDeletePath)
  ...
  : '';
const messageTombstone = fs.existsSync(messageTombstonePath)
  ? fs.readFileSync(messageTombstonePath, 'utf8')
  : '';
const failures = [];
...
requireInvariant(
  /from\('players'\)[\s\S]*\.eq\('account_id', accountId\)/i.test(
    accountDelete,
  ) &&
    /buildMessageSenderIdentityFilterV1\(profileId, playerId\)/i.test(
      accountDelete,
    ) &&
    /sender_player_id_v1\.eq\.\$\{playerId\}/i.test(messageTombstone),
  'account deletion must tombstone messages through authoritative PlayerId, not only legacy profile identity',
);
requireInvariant(
  /buildMessageRemovalTombstoneV1\(deletedAt\)/i.test(accountDelete) &&
    /content_kind_v1: 'system'/i.test(messageTombstone) &&
    /eventType: 'message_removed'/i.test(messageTombstone) &&
    /media_asset_id_v1: null/i.test(messageTombstone) &&
    !/from\('messages'\)\.delete\(/i.test(accountDelete),
